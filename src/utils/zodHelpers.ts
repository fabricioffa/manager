...
export const isValidMobileRefiner = [
  validateMobile,
  { message: 'Celular inválido. Terceiro dígito incorreto' },
] as const;
export const isRepetitionRefiner = [
  isRepetition,
  { message: 'Caracteres repetidos são inválidos' },
] as const;

export const returnIssueToPath = <
  ...
  public validateCtrlDigit(position: 'first' | 'second') {
    ...
    if (validCtrlDigit !== this.cpf[ctrlDigitPosition])
      this.result = {
        isCPF: false,
        message: `${
          position === 'first' ? 'Penúltimo' : 'Último'
        } número inválido. Deveria ser: ${validCtrlDigit}`,
      };
  }
    ...
    if (Array.isArray(dirtyField)) {
      fieldValue = fieldValue.filter(
        (item: object, i: number) =>
          dirtyField.at(i) &&
          Object.values(dirtyField.at(i)).some((value) => value)
      );
    }
