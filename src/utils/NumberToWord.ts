...
export default class NumberToWord {
  ...
  public format(n: number) {
    ...
    if (n.toFixed(2).length > this.maxDigitsCount)
      throw new Error(
        `NumberToWord currently supports only number ranging from 0 to ${'9'.repeat(
          this.maxDigitsCount
        )}.`
      );
    ...
    return partsTextsWithCurrency
      .reverse()
      .join(' ')
      .replace(/(\s+,\s+)|\s+/gi, (match, group1) => (group1 ? ', ' : ' '));
  }
  ...
  private addCurrency(
    partsTexts: string[],
    currencyText: 'de reais' | 'reais'
  ) {
    const partIndex = partsTexts.at(-1)?.match(/centavo|centavos/)
      ? partsTexts.length - 2
      : partsTexts.length - 1;
    return partsTexts.map((part, i) =>
      i === partIndex ? `${part} ${currencyText}` : part
    );
  }
  ...
  private getPartsText(numberParts: string[]) {
    return myObjKeys(this.numberClasses)
      ...
      .map((nClass, i) => {
        const firstNonZeroHigherClassPart = numberParts
          .slice(i + 1)
          .find((part) => part !== '000');
        if (
          this.areJoinedByConjunction(
            firstNonZeroHigherClassPart,
            numberParts[i],
            nClass
          )
        )
        ...
        if (
          numberParts[i] !== '000' &&
          firstNonZeroHigherClassPart &&
          !['decimals', 'units'].includes(nClass)
        )
  ...
  private areJoinedByConjunction(
    higherClassDigits = '000',
    lowerClassDigits = '000',
    lowerClass: NumberClasses
  ) {
    return (
      ![higherClassDigits, lowerClassDigits].includes('000') &&
      ['decimals', 'units'].includes(lowerClass) &&
  ...
  private formatBlock(numberClass: NumberClasses, digits: string) {
    ...
    const blockTextWhithoutUnit = this.joinDigitsTexts(
      digitsTexts,
      this.digitshaveConjunction(digits)
    );
  ...
  private addUnit(numberStr: string, numberClass: NumberClasses) {
    return `${numberStr} ${
      this.numberClasses[numberClass][
        numberStr === 'um' ? 'singular' : 'plural'
      ]
    }`;
  }
}
