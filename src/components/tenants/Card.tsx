  ...
  return (
    <li className="grid bg-tenant border rounded-md shadow-inner text-lg p-4">
      <div className="grid grid-cols-[1.5rem_1fr] grid-rows-[3rem_auto] items-center gap-2">
        ...
        <i className="fa-solid fa-calendar-day fa-xl justify-self-center"></i>
        <p>Vencimentos: 
        ...
        <p className="flex gap-2"> Contratos atuais:
          {
            tenant.contracts.map((contract, i) => (
              <Link className="hover:text-link hover:underline"  href={`/contratos/${contract.id}`} key={contract.id}>#{i + 1}</Link>
            ))
          }
        </p>
        ...
        <p className="flex gap-2"> Casas atuais:
          {
            tenant.contracts.map(({house}, i) => (
              <Link className="hover:text-link hover:underline"  href={`/casas/${house.street}?id=${house.id}`} key={house.id}>#{i + 1}</Link>
            ))
          }
        </p>
        <i className="fa-solid fa-droplet fa-xl justify-self-center"></i>
        <p className="line-clamp-1">{ tenant.waterId }</p>
        <i className="fa-solid fa-bolt fa-xl justify-self-center"></i>
