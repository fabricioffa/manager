...
const Form = ({ contract, action }: FormProps) => {
  ...
  const onValid: SubmitHandler<ContractsSchema> = (rawData, e) => {
    if (action === 'edit' && contract) {
      ...
      edit.mutate(
        { contractData, witnessesData, contractId: contract.id },
        {
          onSuccess() {
              ...
              >
                ...
                {houses.map((house) => (
                  <option
                    value={house.id}
                    key={house.id}
                  >{`${house.street}, ${house.number}`}</option>
                ))}
              </select>
            </InputContainer>
          )}
              ...
              >
                <input
                  ...
                  {...register(`witnesses.${index}.primaryPhone` as const, {
                    onChange: formatOnChange<ContractsSchema>({
                      field: `witnesses.${index}.primaryPhone`,
              ...
              >
                <input
                  ...
                  {...register(`witnesses.${index}.secondaryPhone` as const, {
                    onChange: formatOnChange<ContractsSchema>({
                      field: `witnesses.${index}.secondaryPhone`,
                      formatFunc: formatPhone,
                      setValue: setValue,
                    }),
                  })}
