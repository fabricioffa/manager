...
const updateDb = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    if (req.method !== 'GET') return res.status(405).json({ message: 'Method Not Allowed. Only GET is allowed for this resource.' })

    try {
        const createDebitsResp = await caller.contracts.createDebits()
        const debitsUpdateResp = await caller.debits.update()
        res.status(200).json({ MESSAGE: 'Deu Bom', createDebitsResp, debitsUpdateResp, apiHeaders });
    } catch (cause) {
