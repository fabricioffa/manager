...
export const createPixKeysSchema = z.object({
  ...
  .superRefine(({ key, keyType }, ctx) => {
    ...
    if (keyType === 'cpf')
      returnIssueToPath(cpf.safeParse(key), ctx, 'key')

    if (keyType === 'cnpj')
      returnIssueToPath(cnpj.safeParse(key), ctx, 'key')

    if (keyType === 'aleatoria')
      returnIssueToPath(z.string().trim().length(32).safeParse(key), ctx, 'key')
  })
